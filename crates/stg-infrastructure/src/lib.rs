...
use std::sync::Arc;
use stg_application::{
    ConversionRepository, EnergyRepository, EventLogRepository, PlayerRepository, QueuePublisher, TransactionRepository,
    WalletRepository,
};
use stg_domain::{
    DomainError, DomainEvent, EconomyTransaction, EconomyTransactionType, EnergyMode, EnergyNode, EnergyNodeType,
    EnergyState, LedgerEntry, Money, Player, PlayerId, PlayerStatus, Wallet, WalletId,
    TransactionId, ResourceConversionReservation, ReservationId, ConversionDirection, ReservationStatus,
    ConversionRule,
};
use uuid::Uuid;
...
impl PlayerRepository for PostgresPlayerRepository {
    async fn find_by_id(&self, id: PlayerId) -> Result<Option<Player>, DomainError> {
        ...
        match row {
            Some(r) => {
                let id_val: Uuid = r.try_get("id").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let username_val: String = r.try_get("username").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let status_str: String = r.try_get("status").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let created_at_val: DateTime<Utc> = r.try_get("created_at").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let last_seen_at_val: DateTime<Utc> = r.try_get("last_seen_at").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let revision_val: i64 = r.try_get("revision").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                ...
                let status = match status_str.as_str() {
                    ...
                    _ => return Err(DomainError::InternalStateError("Invalid status in DB".to_string())),
                };
...
impl WalletRepository for PostgresWalletRepository {
    async fn find_by_id(&self, id: WalletId) -> Result<Option<Wallet>, DomainError> {
        ...
        match row {
            Some(r) => {
                let id_val: Uuid = r.try_get("id").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let player_id_val: Uuid = r.try_get("player_id").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let currency_code_val: String = r.try_get("currency_code").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let balance_val: i64 = r.try_get("balance").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let revision_val: i64 = r.try_get("revision").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
...
impl TransactionRepository for PostgresTransactionRepository {
    async fn commit_transaction_with_balances(
        ...
        for wallet in updated_wallets {
            ...
            let locked_row = sqlx::query(
                "SELECT revision FROM wallets WHERE id = $1 FOR UPDATE"
            )
            .bind(wallet.id.0)
            .fetch_one(&mut *tx)
            .await
            ...
            let locked_revision: i64 = locked_row.try_get("revision")
                .map_err(|e| DomainError::InternalStateError(e.to_string()))?;
    ...
    async fn execute_transfer_atomic(
        ...
        if let Some(r) = row {
            let op_type: String = r.try_get("operation_type").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
            let stored_fingerprint: String = r.try_get("request_fingerprint").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
            let payload: String = r.try_get("response_payload").map_err(|e| DomainError::InternalStateError(e.to_string()))?;

            if op_type != "PLAYER_TRANSFER" {
                return Err(DomainError::IdempotencyConflict {
                    request_id: idempotency_key,
                    details: format!("Conflict: request was previously run as a different operation type: {}", op_type),
                });
            }
            ...
            if stored_fingerprint != fingerprint {
                return Err(DomainError::IdempotencyConflict {
                    request_id: idempotency_key,
                    details: "Conflict: duplicate request ID used with a different payload fingerprint".to_string(),
                });
            }

            let transaction: EconomyTransaction = serde_json::from_str(&payload)
                .map_err(|e| DomainError::InternalStateError(e.to_string()))?;
            tx.commit().await.map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        ...
        let source_wallet_id: Uuid = source_wallet_data.try_get("id").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        let dest_wallet_id: Uuid = dest_wallet_data.try_get("id").map_err(|e| DomainError::InternalStateError(e.to_string()))?;

        if source_wallet_id == dest_wallet_id {
            return Err(DomainError::InternalStateError("Cannot transfer money to the same wallet".to_string()));
        }
        ...
        let first_wallet_row = sqlx::query("SELECT id, balance, revision FROM wallets WHERE id = $1 FOR UPDATE")
            .bind(first_lock_id)
            .fetch_one(&mut *tx)
            .await
            .map_err(|e| DomainError::InternalStateError(e.to_string()))?;

        let second_wallet_row = sqlx::query("SELECT id, balance, revision FROM wallets WHERE id = $1 FOR UPDATE")
            .bind(second_lock_id)
            .fetch_one(&mut *tx)
            .await
            .map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        ...
        let locked_source_row = if source_wallet_id == first_lock_id { &first_wallet_row } else { &second_wallet_row };
        let locked_dest_row = if dest_wallet_id == first_lock_id { &first_wallet_row } else { &second_wallet_row };

        let source_balance: i64 = locked_source_row.try_get("balance").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        let source_revision: i64 = locked_source_row.try_get("revision").map_err(|e| DomainError::InternalStateError(e.to_string()))?;

        let dest_balance: i64 = locked_dest_row.try_get("balance").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        let dest_revision: i64 = locked_dest_row.try_get("revision").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        ...
        let new_source_balance = source_balance.checked_sub(amount_minor)
            .ok_or(DomainError::ArithmeticOverflow)?;
        let new_dest_balance = dest_balance.checked_add(amount_minor)
            .ok_or(DomainError::ArithmeticOverflow)?;
    ...
    async fn find_by_id(&self, _id: TransactionId) -> Result<Option<EconomyTransaction>, DomainError> {
        Ok(None)
    }

    async fn is_idempotent_processed(&self, key: Uuid) -> Result<bool, DomainError> {
        let row = sqlx::query(
            "SELECT count(*) as count FROM processed_requests WHERE request_id = $1"
        )
        .bind(key)
        .fetch_one(&self.pool)
        .await
        ...
        let count: i64 = row.try_get("count").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        Ok(count > 0)
    }
...
impl ConversionRepository for PostgresConversionRepository {
    async fn find_rule(&self, namespace: &str, path: &str, direction: ConversionDirection) -> Result<Option<ConversionRule>, DomainError> {
        ...
        match row {
            Some(r) => {
                let id: Uuid = r.try_get("id").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let dir_str: String = r.try_get("direction").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let resource_namespace: String = r.try_get("resource_namespace").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let resource_path: String = r.try_get("resource_path").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let currency_code: String = r.try_get("currency_code").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let unit_price_minor: i64 = r.try_get("unit_price_minor").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let min_amount: i64 = r.try_get("min_amount").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let max_amount: i64 = r.try_get("max_amount").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let enabled: bool = r.try_get("enabled").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let pricing_revision: i64 = r.try_get("pricing_revision").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
    ...
    async fn find_reservation_by_id(&self, id: ReservationId) -> Result<Option<ResourceConversionReservation>, DomainError> {
        let row = sqlx::query(
            "SELECT * FROM resource_conversion_reservations WHERE id = $1"
        )
        .bind(id.0)
        .fetch_optional(&self.pool)
        .await
        ...
        match row {
            Some(r) => {
                ...
                let resource_namespace: String = r.try_get("resource_namespace").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let resource_path: String = r.try_get("resource_path").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let resource_amount: i64 = r.try_get("resource_amount").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let currency_code: String = r.try_get("currency_code").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let unit_price_minor: i64 = r.try_get("unit_price_minor").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let total_price_minor: i64 = r.try_get("total_price_minor").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let pricing_revision: i64 = r.try_get("pricing_revision").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let expires_at: DateTime<Utc> = r.try_get("expires_at").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
    ...
    async fn abort_reservation(&self, id: ReservationId) -> Result<ResourceConversionReservation, DomainError> {
        let mut tx = self.pool.begin().await.map_err(|e| DomainError::InternalStateError(e.to_string()))?;

        // Lock reservation FOR UPDATE
        let row = sqlx::query("SELECT status FROM resource_conversion_reservations WHERE id = $1 FOR UPDATE")
            .bind(id.0)
            .fetch_optional(&mut *tx)
            .await
            .map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        ...
        if status_str == "COMMITTED" {
            return Err(DomainError::ConversionReservationInvalidState("Cannot abort COMMITTED reservation".to_string()));
        }

        if status_str == "PREPARED" {
            sqlx::query("UPDATE resource_conversion_reservations SET status = 'ABORTED' WHERE id = $1")
                .bind(id.0)
                .execute(&mut *tx)
                .await
                .map_err(|e| DomainError::InternalStateError(e.to_string()))?;
    ...
    async fn commit_conversion_atomic(
        ...
        // Lock reservation FOR UPDATE
        let res_row = sqlx::query("SELECT * FROM resource_conversion_reservations WHERE id = $1 FOR UPDATE")
        ...
        let status_str: String = res_row.try_get("status").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        let expires_at: DateTime<Utc> = res_row.try_get("expires_at").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        let player_id: Uuid = res_row.try_get("player_id").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        let amount_minor: i64 = res_row.try_get("total_price_minor").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        let currency_code: String = res_row.try_get("currency_code").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        let dir_str: String = res_row.try_get("direction").map_err(|e| DomainError::InternalStateError(e.to_string()))?;

        if status_str != "PREPARED" {
            return Err(DomainError::ConversionReservationInvalidState(format!("Cannot commit reservation in state {}", status_str)));
        }

        if Utc::now() > expires_at {
            // Update to expired
            sqlx::query("UPDATE resource_conversion_reservations SET status = 'EXPIRED' WHERE id = $1")
                .bind(reservation_id.0)
                .execute(&mut *tx)
                .await
                .map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        ...
        let wallet_id: Uuid = wallet_row.try_get("id").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        let balance: i64 = wallet_row.try_get("balance").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        let revision: i64 = wallet_row.try_get("revision").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        ...
        let new_balance = if is_debit {
            ...
            balance.checked_sub(amount_minor).ok_or(DomainError::ArithmeticOverflow)?
        } else {
            balance.checked_add(amount_minor).ok_or(DomainError::ArithmeticOverflow)?
        };
        ...
        let entries = vec![
            LedgerEntry {
                ...
                amount_delta: if is_debit { -amount_minor } else { amount_minor },
            },
            LedgerEntry {
        ...
        sqlx::query("UPDATE resource_conversion_reservations SET status = 'COMMITTED' WHERE id = $1")
            .bind(reservation_id.0)
            .execute(&mut *tx)
            .await
            .map_err(|e| DomainError::InternalStateError(e.to_string()))?;
        ...
        let event = stg_domain::DomainEvent {
            ...
            aggregate_version: 1,
            payload: stg_domain::DomainEventPayload::MoneyTransferred {
                from_wallet: if is_debit { wallet_id } else { system_wallet_id },
                to_wallet: if is_debit { system_wallet_id } else { wallet_id },
...
impl EnergyRepository for PostgresEnergyRepository {
    async fn find_node_by_id(&self, id: Uuid) -> Result<Option<EnergyNode>, DomainError> {
        ...
        match row {
            Some(r) => {
                let id_val: Uuid = r.try_get("id").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let node_type_str: String = r.try_get("node_type").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let server_id: String = r.try_get("server_id").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let region_id: String = r.try_get("region_id").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let display_name: String = r.try_get("display_name").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let enabled: bool = r.try_get("enabled").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let capacity_watts: i64 = r.try_get("capacity_watts").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let production_watts: i64 = r.try_get("production_watts").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let consumption_watts: i64 = r.try_get("consumption_watts").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let stored_wh: i64 = r.try_get("stored_wh").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let max_stored_wh: i64 = r.try_get("max_stored_wh").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let efficiency: f64 = r.try_get("efficiency").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let health: f64 = r.try_get("health").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let revision_val: i64 = r.try_get("revision").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                let last_reported_at: DateTime<Utc> = r.try_get("last_reported_at").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
                ...
                let node_type = match node_type_str.as_str() {
                    ...
                    _ => return Err(DomainError::InternalStateError("Invalid energy node type in DB".to_string())),
                };
    ...
    async fn update_node(&self, node: &EnergyNode) -> Result<(), DomainError> {
        ...
        if res.rows_affected() == 0 {
            return Err(DomainError::RevisionConflict("EnergyNode".to_string(), node.revision, node.revision));
        }
...
impl OutboxWorker {
    ...
    pub async fn process_next_batch(&self, batch_size: i32) -> Result<usize, DomainError> {
        ...
        for row in &rows {
            let id: Uuid = row.try_get("id").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
            let aggregate_type: String = row.try_get("aggregate_type").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
            let aggregate_id: Uuid = row.try_get("aggregate_id").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
            let payload_json: serde_json::Value = row.try_get("payload_json").map_err(|e| DomainError::InternalStateError(e.to_string()))?;
            let attempt_count: i32 = row.try_get("attempt_count").map_err(|e| DomainError::InternalStateError(e.to_string()))?;

            locked_ids.push(id);
            events_to_publish.push((id, aggregate_type, aggregate_id, payload_json, attempt_count));
        }
                ...
                Ok(event) => {
                    match self.publisher.publish(&event).await {
                        Ok(_) => {
                            ...
                            .execute(&self.pool)
                            .await {
                                eprintln!("Failed to update outbox status to PUBLISHED for event {}: {}", id, e);
                            }
                        }
                        Err(pub_err) => {
                            ...
                            let available_at = Utc::now() + chrono::Duration::seconds(backoff_secs as i64);
                            ...
                            .execute(&self.pool)
                            .await {
                                eprintln!("Failed to update outbox status to FAILED for event {}: {}", id, e);
                            }
                        }
                    }
                }
                Err(serde_err) => {
                    eprintln!("Failed to deserialize outbox payload for {}: {}", id, serde_err);
